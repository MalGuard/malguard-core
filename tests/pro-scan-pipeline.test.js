...
function sandboxMock(overrides = {}) {
  return {
    ...
    analyzeUntrustedSample: async () => ({ ok: false, verdict: 'inconclusive', code: 'HARDENED_SANDBOX_ACCEPTANCE_PENDING' }),
    ...overrides,
  };
}
...
(async()=>{
 ...
 assert.equal(mergeSandboxVerdict('suspicious',{verdict:'safe'}),'suspicious');
 assert.equal(mergeSandboxVerdict('inconclusive',{verdict:'malicious'}),'malicious');
 assert.equal(directSandboxVerdict({verdict:'safe'}),'inconclusive');
 assert.equal(directSandboxVerdict({verdict:'safe',releaseGrade:true}),'safe');
 ...
 const standardManager = new ModelScanPipelineManager({
   ...
   sandbox:sandboxMock({analyzeUntrustedSample:async()=>{standardSandboxCalls++;return {ok:true,verdict:'safe'}}}),
 });
 ...
 const plusSafeManager = new ModelScanPipelineManager({
   ...
   sandbox:sandboxMock({analyzeUntrustedSample:async()=>{plusSandboxCalls++;return {ok:true,verdict:'safe'}}}),
 });
 ...
 const proManager = new ModelScanPipelineManager({
   ...
   sandbox:sandboxMock({analyzeUntrustedSample:async()=>{proSandboxCalls++;return {ok:false,verdict:'inconclusive',code:'HARDENED_SANDBOX_ACCEPTANCE_PENDING'}}}),
 });
 ...
 assert.equal(pro.finalResult.sandboxStarted,true);
 ...
 const proMaliciousManager = new ModelScanPipelineManager({
   ...
   sandbox:sandboxMock({analyzeUntrustedSample:async()=>({ok:true,verdict:'malicious',backend:{backend:'test-sandbox'}})}),
 });
 ...
 console.log('✓ Model scan pipeline: Standard, Plus auto-Sandbox, Pro direct-Sandbox and fail-closed verdict policy passed');
