...
// Explicitly ensure this process exercises the no-license/default path.
...
(async () => {
  ...
  try {
    ...
    assert.equal(plus.body.code, 'ENTITLEMENT_REQUIRED');
    assert.equal(plus.body.requiredPlan, 'plus');
    assert.equal(plus.body.currentPlan, 'standard');

    const pro = await request(port, 'POST', '/api/model-scan/start', { path: safeFile, model: 'pro' });
    assert.equal(pro.status, 403);
    assert.equal(pro.body.code, 'ENTITLEMENT_REQUIRED');
    assert.equal(pro.body.requiredPlan, 'pro');
    assert.equal(pro.body.currentPlan, 'standard');

    const legacy = await request(port, 'POST', '/api/pro-scan/start', { path: safeFile });
    assert.equal(legacy.status, 403);
    assert.equal(legacy.body.code, 'ENTITLEMENT_REQUIRED');
    assert.equal(legacy.body.requiredPlan, 'plus');
  } finally {
    await new Promise(resolve => server.close(resolve));
  }

  console.log('✓ Entitlement API: Standard default allowed, Plus/Pro and legacy paid path fail closed PASS');
