...
const ALLOWED_KEYS=new Set([
  'schemaVersion','event','version','os','build','arch',
  'cpuCoresBucket','ramBucket','language'
]);
...
function parseBody(req){
  ...
  const version=clean(body.version,64);
  const build=clean(body.build,16);
  const language=clean(body.language,24);
  if(!version||!/^[0-9A-Za-z._-]+$/.test(version))throw new Error('invalid-version');
  if(body.os!=='Windows')throw new Error('invalid-os');
  ...
  return {version,os:'Windows',build,arch:body.arch,cpuCoresBucket:body.cpuCoresBucket,ramBucket:body.ramBucket,language};
}
...
function emailText(p){
  return [
    ...
    '',
    'This anonymous compatibility report contains no account name, computer name, hardware serial number, network adapter identifier, precise location, file path, scan result, or installation identifier.'
  ].join('\n');
}
...
module.exports=async function handler(req,res){
  ...
  let payload;
  try{payload=parseBody(req)}catch(_){return res.status(400).json({ok:false,error:'invalid-report'})}
  ...
  try{
    const response=await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({
        from,
        to:[to],
        subject:`New MalGuard installation · ${payload.arch} · Windows`,
        text:emailText(payload),
      }),
    });
    if(!response.ok){
      console.error('install compatibility email provider failed:',response.status);
      return res.status(503).json({ok:false,error:'email-provider-failed'});
    }
    return res.status(200).json({ok:true,notified:true});
  }catch(_){
    console.error('install compatibility email request failed');
