...
const ONLINE_MANIFEST_SCHEMA = '1.1.0';
...
function updatePublicKey(publicKeyPem) {
...
function publicKeyFingerprint(publicKeyPem) {
  const key = updatePublicKey(publicKeyPem);
  const der = key.export({ type: 'spki', format: 'der' });
  return crypto.createHash('sha256').update(der).digest('hex');
}

function verifySignedObject(manifest, signature, publicKeyPem) {
  const key = updatePublicKey(publicKeyPem);
  const signatureBytes = decodeEd25519Signature(signature);
  const ok = crypto.verify(null, Buffer.from(canonicalize(manifest)), key, signatureBytes);
...
function verifyManifest({ manifest, signature, publicKeyPem, currentVersion }) {
  ...
  if (!SHA256_RE.test(manifest.package.sha256)) throw new Error('invalid package sha256');
  if (compareVersions(manifest.version, currentVersion) <= 0) throw new Error('update is not newer than installed version');
  verifySignedObject(manifest, signature, publicKeyPem);
  return true;
}
...
function verifyOnlineUpdateManifest({
  manifest,
  signature,
  publicKeyPem,
  ...
  if (manifest.schemaVersion !== ONLINE_MANIFEST_SCHEMA) throw new Error('unsupported online update manifest schema');
  ...
  if (!crypto.timingSafeEqual(Buffer.from(actualFingerprint, 'hex'), Buffer.from(trustedPublicKeySha256, 'hex'))) {
  ...
  if (compareVersions(manifest.version, currentVersion) <= 0) throw new Error('update is not newer than installed version');
  if (highestSeenVersion && compareVersions(manifest.version, highestSeenVersion) <= 0) throw new Error('update replay or rollback detected');
...
function verifyPackageFile(filePath, expectedSha256, { expectedSize = null, maxBytes = MAX_PACKAGE_BYTES } = {}) {
  ...
  if (expectedSize != null && (!Number.isSafeInteger(expectedSize) || initial.size !== expectedSize)) throw new Error('update package size mismatch');
...
function verifyOnlineUpdatePackage({ filePath, manifest }) {
  if (!manifest || !manifest.package) throw new Error('verified online update manifest is required');
  return verifyPackageFile(filePath, manifest.package.sha256, { expectedSize: manifest.package.size });
}

module.exports = {
  MANIFEST_SCHEMA,
  ONLINE_MANIFEST_SCHEMA,
  PRODUCT_ID,
  MAX_PACKAGE_BYTES,
  canonicalize,
  compareVersions,
